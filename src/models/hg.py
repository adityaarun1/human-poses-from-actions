        ...
        for i in range(self.nStack):
            ...
            lin = nn.Sequential(
                nn.Conv2d(
                    ...
                    bias=True,
                    kernel_size=1,
                    stride=1), nn.BatchNorm2d(self.nFeats), self.relu)
            _lin_.append(lin)
            _tmpOut.append(
                nn.Conv2d(
                    self.nFeats,
...
class PredictionNet(nn.Module):
    def __init__(self, nStack, nModules, nFeats, numOutput):
        ...
        self.convNoiseChannel = nn.Conv2d(
            self.nFeats + 1, self.nFeats, bias=True, kernel_size=1, stride=1)
    ...
    def extract_feats(self, x):
        out = []
        for i in range(self.nStack - 1):  # get feats from all but 1 stacks
            hg = self.hourglass[i](x)
            ll = hg
            for j in range(self.nModules):
                ll = self.Residual[i * self.nModules + j](ll)
    ...
    def forward(self, x, z):
        """
        z is a list of noise samples. Each element is of size (batch, 1, 64, 64)
        """
        x = self.conv1_(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        # extract feats
        out, x = self.extract_feats(x)

        # fast sample
        samples = []
        for noise in z:
            x = torch.cat((x, noise), dim=1)
            x = self.convNoiseChannel(x)
            hg = self.hourglass[-1](x)
            ll = hg
            for j in range(self.nModules):
                ll = self.Residual[(self.nStack - 1) * self.nModules + j](ll)
            ll = self.lin_[(self.nStack - 1)](ll)
            tmpOut = self.tmpOut[(self.nStack - 1)](ll)
            samples.append(tmpOut)

        return (out, samples)


class ConditionalNet(nn.Module):
    def __init__(self, nStack, nModules, nFeats, numOutput, nActionClass):
        super(ConditionalNet, self).__init__()
        self.nStack = nStack
        self.nModules = nModules
        self.nFeats = nFeats
        self.numOutput = numOutput
        self.nActionClass = nActionClass
        ...
        _hourglassAction, _ResidualAction, _linAction_, _tmpOutAction = [], [] ,[], []
        for i in range(self.nActionClass):
            _hourglassAction.append(Hourglass(4, self.nModules, self.nFeats))
            for j in range(self.nModules):
                _ResidualAction.append(Residual(self.nFeats, self.nFeats))
            lin = nn.Sequential(
                nn.Conv2d(
                    self.nFeats,
                    self.nFeats,
                    ...
                    kernel_size=1,
                    stride=1), nn.BatchNorm2d(self.nFeats), self.relu)
            _linAction_.append(lin)
            _tmpOutAction.append(
                nn.Conv2d(
                    self.nFeats,
                    self.numOutput,
                    bias=True,
        ...
        self.ll_ = nn.ModuleList(_ll_)
        self.tmpOut_ = nn.ModuleList(_tmpOut_)
        self.hourglassAction = nn.ModuleList(_hourglassAction)
        self.ResidualAction = nn.ModuleList(_ResidualAction)
        self.linAction_ = nn.ModuleList(_linAction_)
        self.tmpOutAction = nn.ModuleList(_tmpOutAction)
    ...
    def forward(self, x, z, a):
        ...
        for noise in z:
            x = torch.cat((x, noise), dim=1)
            x = self.convNoiseChannel(x)
            hg = self.hourglassAction[a](x)
            ll = hg
            for j in range(self.nModules):
                ll = self.ResidualAction[a * self.nModules + j](ll)
            ll = self.linAction_[a](ll)
            tmpOut = self.tmpOutAction[a](ll)
            samples.append(tmpOut)
