...
def getPreds(hm):
    ...
    for i in range(hm.shape[0]):
        for j in range(hm.shape[1]):
            preds[i, j, 0], preds[i, j, 1] = idx[i, j] % res, old_div(
                idx[i, j], res)
    ...
    for i in range(preds.shape[0]):
        for j in range(preds.shape[1]):
            if gt[i, j, 0] > 0 and gt[i, j, 1] > 0:
                dists[j][i] = old_div(((gt[i][j] - preds[i][j])**2).sum()**0.5,
                                      normalize[i])
            else:
                dists[j][i] = -1
    return dists
...
def Accuracy(output, target):
    ...
    if badIdxCount == len(ref.accIdxs):
        ...
        return old_div(avgAcc, (len(ref.accIdxs) - badIdxCount))


def finalPreds(output, center, scale, rotate):
    p = getPreds(output).copy()
    hm = output.reshape(output.shape[0], output.shape[1], ref.outputRes,
                        ref.outputRes)
    for i in range(hm.shape[0]):
        for j in range(hm.shape[1]):
...
def maximumExpectedUtility(samples, criterion):
    nb_samples = len(samples)
    if nb_samples == 1:
        return samples[0]
    else:
        pairwise_loss = torch.zeros(
            nb_samples, nb_samples, requires_grad=False)

        with torch.no_grad():
            for i in range(nb_samples):
                for j in range(nb_samples):
                    if j > i:
                        pairwise_loss[i][j] = criterion(
                            samples[i].detach(), samples[j].detach())
                    elif j < i:
                        pairwise_loss[i][j] = pairwise_loss[j][i]
                    else:
                        continue
            index = torch.argmin(pairwise_loss.sum(dim=0))
