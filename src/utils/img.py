...
def Crop(img, center, scale, rot, res):
    ...
    if scaleFactor < 2:
        ...
        if newSize < 2:
            return torch.from_numpy(
                old_div(newImg.transpose(2, 0, 1).astype(np.float32), 256.))
        else:
            tmpImg = cv2.resize(tmpImg, (newSize_wd, newSize_ht))  #TODO
            ht, wd = tmpImg.shape[0], tmpImg.shape[1]

    c, s = 1.0 * center / scaleFactor, old_div(scale, scaleFactor)
    c[0], c[1] = c[1], c[0]
    ...
    pad = int(
        np.math.ceil(
            old_div(((
                (ul - br)**2).sum()**0.5), 2) - old_div((br[0] - ul[0]), 2)))
    if rot != 0:
        ul = ul - pad
        br = br + pad
    ...
    try:
        newImg[new_[0]:new_[1], new_[2]:new_[3], :] = tmpImg[
            old_[0]:old_[1], old_[2]:old_[3], :]
    except:
    ...
    if rot != 0:
        M = cv2.getRotationMatrix2D(
            (old_div(newImg.shape[0], 2), old_div(newImg.shape[1], 2)), rot, 1)
...
def Gaussian(sigma):
    if sigma == 7:
        return np.array([
            0.0529, 0.1197, 0.1954, 0.2301, 0.1954, 0.1197, 0.0529, 0.1197,
            0.2709, 0.4421, 0.5205, 0.4421, 0.2709, 0.1197, 0.1954, 0.4421,
            0.7214, 0.8494, 0.7214, 0.4421, 0.1954, 0.2301, 0.5205, 0.8494,
            1.0000, 0.8494, 0.5205, 0.2301, 0.1954, 0.4421, 0.7214, 0.8494,
            0.7214, 0.4421, 0.1954, 0.1197, 0.2709, 0.4421, 0.5205, 0.4421,
            0.2709, 0.1197, 0.0529, 0.1197, 0.1954, 0.2301, 0.1954, 0.1197,
            0.0529
        ]).reshape(7, 7)
    elif sigma == n:
        return g_inp
...
def DrawGaussian(img, pt, sigma):
    ...
    ul = [
        int(np.math.floor(pt[0] - tmpSize)),
    ...
    g_x = [
        max(0, -ul[0]),
        min(br[0], img.shape[1]) - max(0, ul[0]) + max(0, -ul[0])
    ]
    g_y = [
        max(0, -ul[1]),
        min(br[1], img.shape[0]) - max(0, ul[1]) + max(0, -ul[1])
    ]
